...
<form class="dt-form" method="POST" enctype="multipart/form-data" action="{{ route('crm.design_requirements.complete',$ticket->id) }}">{{ csrf_field() }}
    ...
    <div class="dt-field">
        ...
        @foreach($__products as $i => $p)
            @php
                ...
                $__fl = preg_replace('/[^0-9.]/','',$__fp[0] ?? ''); $__fw = preg_replace('/[^0-9.]/','',$__fp[1] ?? ''); $__fh = preg_replace('/[^0-9.]/','',$__fp[2] ?? '');
            @endphp
            <div class="dtp-pane" data-t="{{ $ticket->id }}" data-i="{{ $i }}" style="{{ $i===0 ? '' : 'display:none' }}">
                <div style="border:1px solid #e5eaf1;border-radius:12px;padding:1rem;background:#fbfcfe">
                    ...
                    <div style="display:flex;flex-wrap:wrap;gap:1rem">
                        ...
                        <div>
                            <label style="display:block;margin-bottom:.35rem;color:#536277;font-size:.7rem;font-weight:800">Flat Size (L × W × H) <small style="color:#94a3b8;font-weight:500">optional</small></label>
                            <div style="display:flex;gap:.4rem"><input class="dt-control" style="width:90px;text-align:center" name="product_sizes[{{ $p->id }}][flat_l]" type="number" step="0.01" min="0.01" placeholder="L" value="{{ $__fl }}"><input class="dt-control" style="width:90px;text-align:center" name="product_sizes[{{ $p->id }}][flat_w]" type="number" step="0.01" min="0.01" placeholder="W" value="{{ $__fw }}"><input class="dt-control" style="width:90px;text-align:center" name="product_sizes[{{ $p->id }}][flat_h]" type="number" step="0.01" min="0.01" placeholder="H" value="{{ $__fh }}"></div>
                        </div>
                    </div>
                </div>
            </div>
        @endforeach
        <div class="dt-muted" style="margin-top:.35rem">Fill each product tab's own Open Size (and optional Flat Size) — the estimator receives a separate size for each product.</div>
    </div>
    @endif
    ...
    <div class="dt-field"><label>Flat Size <small style="color:#94a3b8;font-weight:500;">(optional — if left blank, Open Size is used)</small></label><div class="dt-open-size"><div class="dt-open-input"><input class="dt-control" name="flat_length" type="number" step="0.01" min="0.01" inputmode="decimal" placeholder="Length" value="{{ old('flat_length') }}"><span>L</span></div><div class="dt-open-input"><input class="dt-control" name="flat_width" type="number" step="0.01" min="0.01" inputmode="decimal" placeholder="Width" value="{{ old('flat_width') }}"><span>W</span></div><div class="dt-open-input"><input class="dt-control" name="flat_height" type="number" step="0.01" min="0.01" inputmode="decimal" placeholder="Height" value="{{ old('flat_height') }}"><span>H</span></div><span style="align-self:center;font-size:.75rem;color:#94a3b8;">same unit</span></div></div>
    @endif
