...
<body class="{{ $__isAlMassaWorkspace ? 'al-massa-crm' : 'my-box-crm' }}">
    ...
    <script>
        // Global: auto-compress images to KBs before upload + enforce optional [data-max] file count.
        (function () {
            function crmCompressInput(input) {
            ...
            document.addEventListener('change', function (e) {
                ...
                if (el.dataset && el.dataset.max) {
                    var max = parseInt(el.dataset.max, 10) || 0;
                    if (max > 0 && el.files.length > max) { alert('Maximum ' + max + ' files allowed here. Please select fewer.'); el.value = ''; return; }
                }
                if (el.dataset && el.dataset.nocompress !== undefined) return; // opt-out hook
                crmCompressInput(el);
            }, true);
        })();
    </script>
