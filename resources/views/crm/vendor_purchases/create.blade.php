...
<div class="vpc-field vpc-3"><label>Expense Type <span class="vpc-required">*</span></label><select class="vpc-control vpc-item-etype" name="items[{{ $index }}][expense_type]" onchange="vpcItemTypeChanged(this)">@foreach(['Production Expense'=>'Production','Consumable Expense'=>'Consumable','Admin/General Expense'=>'Admin/General'] as $etv=>$etl)<option value="{{ $etv }}" {{ ($item['expense_type'] ?? 'Production Expense')===$etv?'selected':'' }}>{{ $etl }}</option>@endforeach</select></div>
<div class="vpc-field vpc-6"><label>Category <span class="vpc-required">*</span></label><div class="vpc-combo"><input class="vpc-control" name="items[{{ $index }}][category]" value="{{ $item['category'] ?? 'Paper & Board Stock' }}" autocomplete="off" onfocus="openItemCategories(this,true)" oninput="openItemCategories(this,false)" required><button class="vpc-combo-toggle" type="button" onclick="toggleItemCategories(this,event)" aria-label="Show all categories"><i class="fas fa-chevron-down"></i></button><div class="vpc-combo-menu">@foreach($__vpCats as $__cm => $__catList)@foreach($__catList as $category)<button type="button" data-mode="{{ $__cm }}" data-value="{{ $category }}" onclick="chooseItemCategory(this)">{{ $category }}</button>@endforeach @endforeach</div></div><span class="vpc-help">Select or type a custom category.</span></div>
<div class="vpc-field vpc-3 vpc-jobid"><label style="white-space:nowrap">Job ID <span style="font-weight:600;color:#94a3b8;font-size:.7rem">(optional)</span></label><input class="vpc-control" name="items[{{ $index }}][extra][job_id]" value="{{ $item['extra']['job_id'] ?? '' }}" placeholder="e.g. JOB-1024 / INQ-0312"></div><div class="vpc-field vpc-12"><label class="vpc-item-name-label">Description <span class="vpc-required">*</span></label><input class="vpc-control" name="items[{{ $index }}][item_name]" value="{{ $item['item_name'] ?? '' }}" required></div>
<div class="vpc-extra" data-index="{{ $index }}" data-extra="{{ isset($item['extra']) ? (is_array($item['extra']) ? json_encode($item['extra']) : $item['extra']) : '{}' }}" style="grid-column:1/-1;display:grid;grid-template-columns:repeat(12,minmax(0,1fr));gap:.85rem"></div>
<input type="hidden" name="items[{{ $index }}][unit]" value="{{ $item['unit'] ?? 'Items' }}">
<div class="vpc-field vpc-12"><div class="vpc-price-grid"><div class="vpc-field"><label>Qty <span class="vpc-required">*</span></label><input class="vpc-control vpc-item-calc" data-role="quantity" type="number" step=".01" min=".01" name="items[{{ $index }}][quantity]" value="{{ $item['quantity'] ?? 1 }}" required></div><div class="vpc-field"><label>P/Unit <span class="vpc-required">*</span></label><input class="vpc-control vpc-item-calc" data-role="unit-price" type="number" step=".0001" min="0" name="items[{{ $index }}][unit_price]" value="{{ $item['unit_price'] ?? '' }}" required></div><div class="vpc-field"><label>Total</label><input class="vpc-control" data-role="line-total" type="number" step=".01" min="0" name="items[{{ $index }}][line_total]" value="{{ $item['line_total'] ?? '' }}" readonly></div><div class="vpc-field"><label>VAT %</label><input class="vpc-control vpc-item-calc" data-role="vat" type="number" step=".01" min="0" max="100" name="items[{{ $index }}][vat_percentage]" value="{{ $item['vat_percentage'] ?? 0 }}" placeholder="e.g. 5"></div><div class="vpc-field"><label>Gross</label><input class="vpc-control" data-role="gross" type="number" step=".01" min="0" readonly value="{{ isset($item['line_total']) ? number_format((float)$item['line_total'] * (1 + ((float)($item['vat_percentage'] ?? 0))/100), 2, '.', '') : '' }}"></div></div></div>
</div></div>
@endforeach
</div>
