...
<div class="vpc-field vpc-3"><label id="vpcVendorLabel">Vendor <span class="vpc-required">*</span></label><select class="vpc-control" name="vendor_id" onchange="fillVendor(this)" required><option value="">Choose saved vendor / payee</option>@foreach($vendors as $vendor)<option value="{{ $vendor->id }}" data-phone="{{ $vendor->phone }}" data-email="{{ $vendor->email }}" data-category="{{ $vendor->category }}" data-vtype="{{ $vendor->vendor_type }}" data-vtypelabel="{{ $vendor->typeLabel() }}" {{ (int)old('vendor_id',$selectedVendorId)===$vendor->id?'selected':'' }}>{{ $vendor->name }} ({{ $vendor->typeLabel() }})</option>@endforeach</select><div id="vpcVtypeHint" style="display:none;margin-top:.35rem;font-size:.68rem;color:var(--primary-purple);font-weight:700"></div></div>
...
<div class="vpc-field vpc-3"><label>Purchase Date <span class="vpc-required">*</span></label><input class="vpc-control" type="date" name="purchase_date" value="{{ old('purchase_date', $isEditingPurchase ? optional($purchase->purchase_date)->format('Y-m-d') : date('Y-m-d')) }}" required></div><div class="vpc-field vpc-3"><label>Invoice Number</label><input class="vpc-control" name="invoice_number" value="{{ old('invoice_number', $isEditingPurchase ? $purchase->invoice_number : '') }}"></div><div class="vpc-field vpc-3"><label>Job ID</label><input class="vpc-control" name="job_id" value="{{ old('job_id', $isEditingPurchase ? $purchase->job_id : '') }}" placeholder="e.g. JOB-1024 / INQ-0312"></div><div class="vpc-field vpc-3"><label>Demand Request <span class="vpc-opt">(optional)</span></label><select class="vpc-control" name="demand_id"><option value="">— none —</option>@foreach(($demandOptions ?? []) as $opt)<option value="{{ $opt['id'] }}" {{ (int)old('demand_id', $isEditingPurchase ? $purchase->demand_id : null)===(int)$opt['id']?'selected':'' }}>{{ $opt['label'] }}</option>@endforeach</select></div><div class="vpc-field vpc-3" id="vpcUpField"><label>Up / Imposition <span class="vpc-opt">(CTP)</span></label><input class="vpc-control" name="up_imposition" value="{{ old('up_imposition', $isEditingPurchase ? $purchase->up_imposition : '') }}" placeholder="e.g. 4-up"></div><div class="vpc-field vpc-3"><label>GP Status <span class="vpc-opt">(optional)</span></label><input class="vpc-control" name="gp_status" value="{{ old('gp_status', $isEditingPurchase ? $purchase->gp_status : '') }}" placeholder="e.g. Received / Pending"></div>
...
var VPC_TYPE_FIELDS={paper:'Paper Type · GSM · Size · Unit · Sheet Qty · Rate · GST',ctp_plate:'Up / Imposition · Rate',die_making:'Size · Colours · Rate',general:'Description · Amount'};
function vpcApplyVendorType(vtype,vlabel){var up=document.getElementById('vpcUpField');if(up)up.style.display=(vtype==='ctp_plate')?'':'none';var hint=document.getElementById('vpcVtypeHint');if(hint){if(vtype){hint.style.display='';hint.textContent=(vlabel||'')+' vendor — fill: '+(VPC_TYPE_FIELDS[vtype]||'');}else{hint.style.display='none';}}}
function fillVendor(select){var option=select.options[select.selectedIndex],form=document.getElementById('vendorPurchaseCreateForm');form.elements.vendor_phone.value=option?option.dataset.phone||'':'';form.elements.vendor_email.value=option?option.dataset.email||'':'';vpcApplyVendorType(option?option.dataset.vtype:'',option?option.dataset.vtypelabel:'');}
document.addEventListener('DOMContentLoaded',function(){var s=document.querySelector('select[name="vendor_id"]');if(s&&s.value)fillVendor(s);else vpcApplyVendorType('','');});
