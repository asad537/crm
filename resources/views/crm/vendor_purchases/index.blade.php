...
<div class="vp-page">
    @if(!$selectedVendor)
    @php
        // Expense type is per line item now (one invoice can mix Production + Consumable), so the
        ...
        $expensePurchases = $__wsP->flatMap(function ($p) {
            ...
            if ($p->items->isEmpty()) {
                $row = new \stdClass();
                $row->expense_category = $p->expense_type ?: 'Production Expense';
                $row->total_amount = (float) $p->total_amount;
                $row->paid_amount = (float) $p->paid_amount;
                $row->balance_amount = (float) $p->balance_amount;
                $row->purchase_date = $p->purchase_date;
                return [$row];
            }
            // Split the purchase total/paid/balance across its items by each item's gross share, so
            // every item lands under its own expense type while the amounts still sum to the invoice.
            $grosses = $p->items->map(function ($it) { return (float) $it->line_total * (1 + (float) $it->vat_percentage / 100); });
            $sumGross = $grosses->sum();
            $count = $grosses->count();
            return $p->items->values()->map(function ($it, $i) use ($p, $grosses, $sumGross, $count) {
                $ratio = $sumGross > 0 ? (float) $grosses[$i] / $sumGross : ($count ? 1 / $count : 0);
                $row = new \stdClass();
                $row->expense_category = $it->expense_type ?: 'Production Expense';
                $row->total_amount = round((float) $p->total_amount * $ratio, 2);
                $row->paid_amount = round((float) $p->paid_amount * $ratio, 2);
                $row->balance_amount = round((float) $p->balance_amount * $ratio, 2);
                $row->purchase_date = $p->purchase_date;
                return $row;
            });
        });
        $expenseTotal = (float) $expensePurchases->sum('total_amount');
        $productionTotal = (float) $expensePurchases->where('expense_category', 'Production Expense')->sum('total_amount');
        $personalTotal = (float) $expensePurchases->where('expense_category', 'Consumable Expense')->sum('total_amount');
        $generalTotal = (float) $expensePurchases->where('expense_category', 'Admin/General Expense')->sum('total_amount');
        $productionPaid = (float) $expensePurchases->where('expense_category', 'Production Expense')->sum('paid_amount');
        $productionUnpaid = (float) $expensePurchases->where('expense_category', 'Production Expense')->sum('balance_amount');
        $personalPaid = (float) $expensePurchases->where('expense_category', 'Consumable Expense')->sum('paid_amount');
        $personalUnpaid = (float) $expensePurchases->where('expense_category', 'Consumable Expense')->sum('balance_amount');
        $generalPaid = (float) $expensePurchases->where('expense_category', 'Admin/General Expense')->sum('paid_amount');
        $generalUnpaid = (float) $expensePurchases->where('expense_category', 'Admin/General Expense')->sum('balance_amount');
        ...
        $expenseDaily = $expensePurchases->filter(function ($purchase) { return (bool) $purchase->purchase_date; })->groupBy(function ($purchase) { return $purchase->purchase_date->format('Y-m-d'); })->map(function ($rows) { return ['production' => (float) $rows->where('expense_category', '!=', 'Consumable Expense')->sum('total_amount'), 'personal' => (float) $rows->where('expense_category', 'Consumable Expense')->sum('total_amount')]; });
        ...
        $productionChange = $expenseChange((float) $thisMonthExpenses->where('expense_category', 'Production Expense')->sum('total_amount'), (float) $lastMonthExpenses->where('expense_category', 'Production Expense')->sum('total_amount'));
        $personalChange = $expenseChange((float) $thisMonthExpenses->where('expense_category', 'Consumable Expense')->sum('total_amount'), (float) $lastMonthExpenses->where('expense_category', 'Consumable Expense')->sum('total_amount'));
        $generalChange = $expenseChange((float) $thisMonthExpenses->where('expense_category', 'Admin/General Expense')->sum('total_amount'), (float) $lastMonthExpenses->where('expense_category', 'Admin/General Expense')->sum('total_amount'));
        $pendingChange = $expenseChange((float) $thisMonthExpenses->sum('balance_amount'), (float) $lastMonthExpenses->sum('balance_amount'));
        $paidChange = $expenseChange((float) $thisMonthExpenses->sum('paid_amount'), (float) $lastMonthExpenses->sum('paid_amount'));
    @endphp
    <div class="vp-expense-dashboard">
        <div class="vp-expense-cards">
            ...
            <a href="{{ route('crm.vendor_purchases.index', array_merge($__vpBase, ['expense_type'=>'Consumable Expense'])) }}" class="vp-expense-card vp-expense-card-link {{ request('expense_type')==='Consumable Expense' ? 'is-active' : '' }}" title="Filter: Consumable Expenses"><span class="vp-expense-icon" style="color:#6457b8;background:#efedff"><i class="fas fa-user"></i></span><div class="vp-expense-card-copy"><span class="vp-expense-label">Consumable Expenses</span><strong class="vp-expense-value">{{ number_format($personalTotal, 2) }}</strong><small class="vp-expense-note">vs last month <span class="vp-expense-change {{ $personalChange < 0 ? 'down' : '' }}">{{ $personalChange >= 0 ? '↑' : '↓' }} {{ number_format(abs($personalChange), 1) }}%</span></small><div class="vp-expense-payment-split"><span class="paid"><i class="fas fa-check-circle"></i> Paid {{ number_format($personalPaid, 2) }}</span><span class="unpaid"><i class="fas fa-clock"></i> Unpaid {{ number_format($personalUnpaid, 2) }}</span></div></div></a>
            <a href="{{ route('crm.vendor_purchases.index', array_merge($__vpBase, ['payment_status'=>'Paid'])) }}" class="vp-expense-card vp-expense-card-link {{ request('payment_status')==='Paid' ? 'is-active' : '' }}" title="Filter: Paid"><span class="vp-expense-icon" style="color:#159447;background:#e6f7e9"><i class="fas fa-check-circle"></i></span><div class="vp-expense-card-copy"><span class="vp-expense-label">Total Paid</span><strong class="vp-expense-value">{{ number_format($expensePaid, 2) }}</strong><small class="vp-expense-note">vs last month <span class="vp-expense-change {{ $paidChange < 0 ? 'down' : '' }}">{{ $paidChange >= 0 ? '↑' : '↓' }} {{ number_format(abs($paidChange), 1) }}%</span></small></div></a>
            <a href="{{ route('crm.vendor_purchases.index', array_merge($__vpBase, ['payment_status'=>'Unpaid'])) }}" class="vp-expense-card vp-expense-card-link {{ request('payment_status')==='Unpaid' ? 'is-active' : '' }}" title="Filter: Unpaid / pending"><span class="vp-expense-icon" style="color:#2563eb;background:#e8f1ff"><i class="fas fa-clock"></i></span><div class="vp-expense-card-copy"><span class="vp-expense-label">Total Unpaid</span><strong class="vp-expense-value">{{ number_format($expensePending, 2) }}</strong><small class="vp-expense-note">vs last month <span class="vp-expense-change {{ $pendingChange < 0 ? 'down' : '' }}">{{ $pendingChange >= 0 ? '↑' : '↓' }} {{ number_format(abs($pendingChange), 1) }}%</span></small></div></a>
        </div>
        <div class="vp-expense-grid">
            <div class="vp-expense-panel"><h3>Expense Overview</h3><div class="vp-expense-split"><div class="vp-expense-donut-wrap"><canvas id="vpExpenseDonut"></canvas><div class="vp-expense-donut-center"><strong>{{ number_format($expenseTotal, 2) }}</strong><span>Total</span></div></div><div class="vp-expense-legend"><div><span><i class="vp-expense-legend-dot" style="background:#7acb8a"></i>Production Expenses</span><strong>{{ number_format($productionTotal, 2) }} ({{ number_format($expenseProductionPct, 1) }}%)</strong></div><div><span><i class="vp-expense-legend-dot" style="background:#ff965f"></i>Consumable Expenses</span><strong>{{ number_format($personalTotal, 2) }} ({{ number_format($expensePersonalPct, 1) }}%)</strong></div></div></div></div>
...
<tr class="vp-row-link" data-href="{{ route('crm.vendor_purchases.index',array_merge(request()->only('expense_type'), ['vendor_id'=>$vendor->id])) }}" data-vendor-search="{{ strtolower($vendor->name.' '.$vendor->trn_number.' '.$vendor->phone.' '.$vendor->email) }}"><td>
...
<div class="vp-action-group"><a class="vp-back" href="{{ route('crm.vendor_purchases.index',array_merge(request()->only('expense_type'), ['vendor_id'=>$vendor->id])) }}" title="View vendor purchases"><i class="fas fa-eye"></i></a><button type="button" class="vp-directory-delete" style="border-color:#c7b8f5;color:#6c5ce7;background:var(--primary-soft,#f3f0ff);margin-left:0" title="Edit vendor" data-vendor="{{ json_encode(['id'=>$vendor->id,'name'=>$vendor->name,'vendor_type'=>$vendor->vendor_type,'trn_number'=>$vendor->trn_number,'phone'=>$vendor->phone,'email'=>$vendor->email,'address'=>$vendor->address,'notes'=>$vendor->notes]) }}" onclick="openVendorEditModal(JSON.parse(this.dataset.vendor))"><i class="fas fa-pen"></i></button>@if($canDeleteVendors)<form method="POST" action="{{ route('crm.vendors.destroy',$vendor->id) }}" data-delete-target="{{ $vendor->name }}" onsubmit="return openVendorDeleteDialog(this,'vendor',this.dataset.deleteTarget);">{{ csrf_field() }}{{ method_field('DELETE') }}<button class="vp-directory-delete" type="submit" title="Delete vendor"><i class="fas fa-trash"></i></button></form>@endif</div></td></tr>@empty<tr><td colspan="7" class="vp-empty">No vendors found. Click “Add Vendor” to create one.</td></tr>@endforelse</tbody></table>
    ...
    <div class="vp-card">
        <div class="vp-toolbar-row"><form method="GET" class="vp-toolbar"><input type="hidden" name="vendor_id" value="{{ $selectedVendor->id }}">
            <div class="vp-search"><i class="fas fa-search"></i><input class="vp-control" name="search" id="vpPurchaseLiveSearch" autocomplete="off" value="{{ request('search') }}" placeholder="Search date, item, invoice, job, payment…" oninput="vpPurchaseSearchDebounced(this)"><button class="vp-search-btn" type="submit" title="Search"><i class="fas fa-arrow-right"></i></button></div>
<style>.catf{position:relative;min-width:210px}.catf-ico{position:absolute;left:11px;top:50%;transform:translateY(-50%);color:#94a3b8;font-size:.8rem;pointer-events:none}.catf-input{padding-left:31px!important}.catf-menu{display:none;position:absolute;z-index:130;top:calc(100% + 6px);left:0;right:0;max-height:300px;overflow-y:auto;padding:6px;border:1px solid #dbe3ed;border-radius:11px;background:#fff;box-shadow:0 16px 36px rgba(15,23,42,.18)}.catf-menu.show{display:block}.catf-group{padding:7px 10px 3px;font-size:.6rem;font-weight:850;text-transform:uppercase;letter-spacing:.05em;color:#94a3b8}.catf-opt{display:block;width:100%;padding:8px 10px;border:0;border-radius:7px;background:#fff;color:#334155;font:inherit;font-size:.82rem;text-align:left;cursor:pointer}.catf-opt:hover{background:var(--primary-soft);color:var(--primary-purple)}</style>
            <div class="catf" id="catf"><i class="fas fa-search catf-ico"></i><input type="text" class="vp-control catf-input" id="catfInput" autocomplete="off" placeholder="All categories" value="{{ request('category') }}" oninput="catfFilter(this.value)" onfocus="catfOpen()"><input type="hidden" name="category" id="catfValue" value="{{ request('category') }}"><div class="catf-menu" id="catfMenu"><button type="button" class="catf-opt" data-value="" onclick="catfPick('')"><i class="fas fa-layer-group" style="color:var(--primary-purple);margin-right:6px"></i>All categories</button>@foreach($expenseCatGroups as $__grp => $__cats)<div class="catf-group">{{ $__grp }}</div>@foreach($__cats as $category)<button type="button" class="catf-opt" data-value="{{ $category }}" onclick="catfPick(this.dataset.value)">{{ $category }}</button>@endforeach @endforeach</div></div>
            <script>
            function catfOpen(){document.getElementById('catfMenu').classList.add('show');catfFilter(document.getElementById('catfInput').value);}
            function catfClose(){document.getElementById('catfMenu').classList.remove('show');}
            function catfFilter(q){q=(q||'').toLowerCase();document.querySelectorAll('#catfMenu .catf-opt').forEach(function(b){b.style.display=(!q||(b.dataset.value+' '+b.textContent).toLowerCase().indexOf(q)!==-1)?'block':'none';});document.querySelectorAll('#catfMenu .catf-group').forEach(function(g){g.style.display=q?'none':'block';});}
            function catfPick(v){document.getElementById('catfValue').value=v;document.getElementById('catfInput').value=v;catfClose();document.getElementById('catfValue').form.submit();}
            document.addEventListener('click',function(e){if(!e.target.closest('#catf'))catfClose();});
            </script>
        ...
        <div class="vp-table-wrap"><table class="vp-table"><thead><tr><th>Date</th><th>Vendor</th><th>Vendor Invoice</th><th>Job No</th><th>Demand</th><th>Packaging Item</th><th>Qty</th><th style="text-align:right">Debit</th><th style="text-align:right">Credit</th><th style="text-align:right">Balance</th><th>Attachment</th><th>Actions</th></tr></thead><tbody>
            ...
            @empty
            @if(request('search'))
            <tr><td colspan="{{ $vpColspan ?? 12 }}"><div class="vp-empty"><i class="fas fa-search" style="font-size:1.6rem;margin-bottom:.6rem"></i><div>No records match “{{ request('search') }}”.</div></div></td></tr>
            @else
...
function initVendorExpenseCharts() {
    ...
    if (donut) {
        expenseCharts.push(new Chart(donut, {
            type: 'doughnut',
            data: {
                labels: ['Production Expenses', 'Consumable Expenses'],
                datasets: [{
...
<div id="vendorPurchaseModal" class="vp-modal-backdrop" onclick="if(event.target===this) requestVendorClose('purchase')">
    <div class="vp-modal" role="dialog" aria-modal="true" aria-labelledby="vendorPurchaseTitle">
        ...
        <form class="vp-form" id="vendorPurchaseForm" method="POST" action="{{ route('crm.vendor_purchases.store') }}" enctype="multipart/form-data">{{ csrf_field() }}<input type="hidden" name="_method" id="vpFormMethod" value="POST">
            ...
            <div class="vp-grid">
                <div class="vp-field"><label>Category <span class="vp-required">*</span></label><div class="vp-combobox"><input class="vp-control" name="category" value="Paper & Board Stock" autocomplete="off" oninput="filterVendorCategories(this.value)" onfocus="openVendorCategories()" required><button class="vp-combobox-toggle" type="button" onclick="toggleVendorCategories(event)" aria-label="Show all categories"><i class="fas fa-chevron-down"></i></button><div class="vp-combobox-menu" id="vendorCategoryMenu">@foreach($expenseCatGroups as $__cats)@foreach($__cats as $category)<button class="vp-combobox-option" type="button" data-value="{{ $category }}" onclick="selectVendorCategory(this.dataset.value)">{{ $category }}</button>@endforeach @endforeach</div></div><small class="vp-muted">Select an option or type a custom category.</small></div>
...
<script>
    ...
    function vpPurchaseSearchDebounced(input){clearTimeout(vpSearchTimer);vpSearchTimer=setTimeout(function(){var f=input.form;if(!f)return;var pp=f.querySelector('[name="page"]');if(pp)pp.remove();f.submit();},500);}
    document.addEventListener('DOMContentLoaded',function(){var i=document.getElementById('vpPurchaseLiveSearch');if(i&&i.value){i.focus();var v=i.value;i.value='';i.value=v;}});
    setTimeout(function(){
