...
<div class="vp-page">
    ...
    @php
        ...
        $generalTotal = (float) $expensePurchases->where('expense_category', 'General Expense')->sum('total_amount');
        ...
        $generalPaid = (float) $expensePurchases->where('expense_category', 'General Expense')->sum('paid_amount');
        $generalUnpaid = (float) $expensePurchases->where('expense_category', 'General Expense')->sum('balance_amount');
        ...
        $generalChange = $expenseChange((float) $thisMonthExpenses->where('expense_category', 'General Expense')->sum('total_amount'), (float) $lastMonthExpenses->where('expense_category', 'General Expense')->sum('total_amount'));
    ...
    <div class="vp-expense-dashboard">
        <div class="vp-expense-cards">
            ...
            <a href="{{ route('crm.vendor_purchases.index', array_merge($__vpBase, ['expense_type'=>'General Expense'])) }}" class="vp-expense-card vp-expense-card-link {{ request('expense_type')==='General Expense' ? 'is-active' : '' }}" title="Filter: General Expenses"><span class="vp-expense-icon" style="color:#0891b2;background:#e0f7fb"><i class="fas fa-receipt"></i></span><div class="vp-expense-card-copy"><span class="vp-expense-label">General Expenses</span><strong class="vp-expense-value">{{ number_format($generalTotal, 2) }}</strong><small class="vp-expense-note">vs last month <span class="vp-expense-change {{ $generalChange < 0 ? 'down' : '' }}">{{ $generalChange >= 0 ? '↑' : '↓' }} {{ number_format(abs($generalChange), 1) }}%</span></small><div class="vp-expense-payment-split"><span class="paid"><i class="fas fa-check-circle"></i> Paid {{ number_format($generalPaid, 2) }}</span><span class="unpaid"><i class="fas fa-clock"></i> Unpaid {{ number_format($generalUnpaid, 2) }}</span></div></div></a>
