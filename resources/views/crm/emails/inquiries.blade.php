...
<div class="iq-page">
    ...
    @php
        // Preload proposals for the visible inquiries so the row status can reflect a raised proposal.
        $__proposalMap = \Illuminate\Support\Facades\Schema::hasTable('crm_proposals')
            ? \App\CrmProposal::whereIn('crm_email_id', $inquiries->pluck('id')->filter()->all())->get()->keyBy('crm_email_id')
            : collect();
    @endphp
    @forelse($inquiries as $inquiry)
        ...
        <tr>
            ...
            @php
                // A raised proposal takes over the inquiry status so "Request Proposal" is visible in the list.
                $__inqProposal = $__proposalMap->get($inquiry->id);
                if ($__inqProposal) {
                    $workflowClass = $__inqProposal->status === 'completed' ? 'done' : 'design';
                    $workflowLabel = 'Proposal ' . ucwords(str_replace('_', ' ', $__inqProposal->status));
                }
            @endphp
                        ...
                        <div class="iq-action-dropdown">
                            ...
                            @else
                                <form method="POST" action="{{ route('crm.proposals.request_from_inquiry',$inquiry->id) }}">
                                    {{ csrf_field() }}
                                    <button class="iq-dropdown-item" type="submit"><i class="fas fa-file-signature"></i> {{ $__rowProposal ? 'View Proposal' : 'Request Proposal' }}</button>
                                </form>
                            @endif
                        </div>
                    </details>
