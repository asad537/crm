...
@section('content')
@php
    $__openSizeParts = preg_split('/\s*(?:x|\*|×)\s*/i', (string) old('open_size', $inquiry->open_size));
    ...
    foreach ((array) old('finishing_options', $inquiry->custom_specs['Finishing Options'] ?? []) as $__savedFinish) {
        $__parts = array_map('trim', explode(' — ', $__savedFinish, 2));
        if (($__parts[0] ?? null) === 'Embossing') $__parts[0] = 'Emboss / Deboss';
        if (count($__parts) === 2 && !in_array($__parts[1], $__finishingGroups[$__parts[0]] ?? [], true)) {
            $__finishingGroups[$__parts[0]][] = $__parts[1];
        }
    }
    $__selectedFinishing = array_map(function ($value) {
        return strpos($value, 'Embossing — ') === 0
            ...
            : $value;
    }, (array) old('finishing_options', $inquiry->custom_specs['Finishing Options'] ?? []));
...
<div class="mi-modal">
    <div class="mi-head"><div class="mi-title"><span class="mi-icon"><i class="fas fa-plus-circle"></i></span><div><h2>Edit Inquiry</h2><div class="mi-muted">Customer details, specifications, multiple quantities and files</div></div></div><button class="mi-close" type="button" onclick="window.location='{{ route('crm.inquiries.index') }}'"><i class="fas fa-times"></i></button></div>
    <form class="mi-form" method="POST" action="{{ route('crm.emails.update_inquiry', $inquiry->id) }}" enctype="multipart/form-data">{{ csrf_field() }}
        <div class="mi-table-block">
            <div class="mi-grid mi-client-grid">
                ...
                <div class="mi-cell" data-label="Client Name"><input class="mi-control" name="client_name" value="{{ old('client_name', $inquiry->client_name) }}" required></div>
                <div class="mi-cell" data-label="Client Email"><input class="mi-control" type="email" name="client_email" value="{{ old('client_email', $inquiry->client_email) }}" required></div>
                <div class="mi-cell" data-label="Client Mobile / Phone"><input class="mi-control" name="client_phone" value="{{ old('client_phone', $inquiry->client_phone) }}"></div>
                <div class="mi-cell" data-label="Currency"><select class="mi-control" name="inquiry_currency" required>@foreach(['USD'=>'USD — US Dollar','AED'=>'AED — UAE Dirham','GBP'=>'GBP — British Pound','EUR'=>'EUR — Euro','CAD'=>'CAD — Canadian Dollar','AUD'=>'AUD — Australian Dollar','PKR'=>'PKR — Pakistani Rupee','SAR'=>'SAR — Saudi Riyal','QAR'=>'QAR — Qatari Riyal'] as $code=>$label)<option value="{{ $code }}" {{ old('inquiry_currency', $inquiry->invoice_currency)===$code?'selected':'' }}>{{ $label }}</option>@endforeach</select></div>
        ...
        <div class="mi-table-block mi-product-block">
            <div class="mi-grid mi-product-grid">
                ...
                <div class="mi-cell" data-label="Open Size">
                    ...
                    <input type="hidden" name="open_size" id="openSize" value="{{ old('open_size', $inquiry->open_size) }}">
                </div>
                <div class="mi-cell" data-label="Stock"><input class="mi-control mi-combo" name="stock" list="stockOptions" autocomplete="off" value="{{ old('stock', $inquiry->stock) }}"><datalist id="stockOptions">@foreach(['12pt Cardboard Stock','14pt Cardboard Stock','16pt Cardboard Stock','18pt Cardboard Stock','20pt Cardboard Stock','24pt Cardboard Stock','Kraft Paper Stock','Recycled Kraft Stock','SBS Paperboard','FBB Paperboard','CCNB Paperboard','Corrugated E-Flute','Corrugated B-Flute','Corrugated C-Flute','Double Wall Corrugated','Grey Chipboard','Rigid Board 1.5mm','Rigid Board 2mm','Rigid Board 3mm','Art Paper 128gsm','Art Paper 157gsm','Art Paper 200gsm','Art Paper 250gsm','Art Paper 300gsm','Art Paper 350gsm'] as $option)<option value="{{ $option }}">@endforeach</datalist></div>
                <div class="mi-cell" data-label="Quantity Options"><div class="mi-quantity-list" id="quantityList">
@foreach((old('quantities') ?? ($inquiry->inquiry_quantities ?: [$inquiry->quantity])) as $qty)
<div class="mi-quantity"><input class="mi-control" type="number" name="quantities[]" min="1" placeholder="e.g. 500" value="{{ $qty }}" required><button class="mi-remove" type="button" onclick="removeQuantity(this)"><i class="fas fa-times"></i></button></div>
@endforeach
</div><button class="mi-add" type="button" onclick="addQuantity()"><i class="fas fa-plus"></i> Add Quantity</button></div>
                <div class="mi-cell" data-label="Price Offered"><input class="mi-control" type="number" step=".01" min="0" name="price_offered" value="{{ old('price_offered', $inquiry->price_offered) }}"></div>
            </div>
        </div>

        <div class="mi-extra">
            <div class="mi-field"><label>Inquiry Date *</label><input class="mi-control" type="date" name="inquiry_date" value="{{ old('inquiry_date', $inquiry->created_at->format('Y-m-d')) }}" required></div>
            <div class="mi-field"><label>Inquiry Source *</label><select class="mi-control" name="source" required>@foreach(['website'=>'Website','call'=>'Call','walk_in'=>'Walk-in','social_media'=>'Social Media','whatsapp'=>'WhatsApp','live_chat'=>'Live Chat','email'=>'Email'] as $value=>$label)<option value="{{ $value }}" {{ old('source',in_array($inquiry->source,['website','call','walk_in','social_media','whatsapp','live_chat','email'],true)?$inquiry->source:'website')===$value?'selected':'' }}>{{ $label }}</option>@endforeach</select></div>
            <div class="mi-field"><label>Shipping Address</label><textarea class="mi-control" name="shipping_address" rows="2" placeholder="Street, city, state, postal code, country">{{ old('shipping_address', $inquiry->shipping_address) }}</textarea></div>
            <div class="mi-field"><label>Sales Notes / Requirements</label><textarea class="mi-control" name="message" rows="2">{{ old('message', $inquiry->message) }}</textarea></div>
        ...
        <div class="mi-dialog-backdrop" id="finishingDialog" role="dialog" aria-modal="true"><div class="mi-dialog"><div class="mi-dialog-head"><div class="mi-dialog-title"><i class="fas fa-layer-group"></i><div><h3>Add Finishing Option</h3><small>Type a new parent or choose an existing one</small></div></div><button class="mi-dialog-close" type="button" onclick="closeFinishingDialog()"><i class="fas fa-times"></i></button></div><div class="mi-dialog-body"><div class="mi-dialog-field"><label>Parent Category *</label><input class="mi-control" id="finishingParent" list="finishingParentOptions" maxlength="80" autocomplete="off" placeholder="Type new or select existing parent"><datalist id="finishingParentOptions">@foreach(array_keys($__finishingGroups) as $group)<option value="{{ $group }}">@endforeach</datalist></div><div class="mi-dialog-field"><label>Child Option *</label><input class="mi-control" id="finishingChild" maxlength="100" placeholder="e.g. Pearl Texture"></div></div><div class="mi-dialog-actions"><button class="mi-dialog-btn mi-dialog-cancel" type="button" onclick="closeFinishingDialog()">Cancel</button><button class="mi-dialog-btn mi-dialog-save" type="button" onclick="saveFinishingOption()"><i class="fas fa-check"></i> Add Option</button></div></div></div>
        <div class="mi-actions"><a class="mi-btn mi-btn-soft" href="{{ route('crm.inquiries.index') }}">Cancel</a><button class="mi-btn mi-btn-main" type="submit"><i class="fas fa-save"></i> Save Inquiry Changes</button></div>
    </form>
</div></div>
@endsection
...
document.querySelector('.mi-form').addEventListener('submit',function(){
    ...
    document.getElementById('finishSize').value=paper.join(' x ');
