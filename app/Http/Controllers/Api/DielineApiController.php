...
class DielineApiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // The "Request Design" flow sends the box dimensions and the customer's message
            // with the dieline request. Persist them onto the parent project so the CRM shows
            // the size and notes (only overwrite when actually provided).
            $projectUpdates = [];
            foreach (['width', 'height', 'length', 'unit', 'message'] as $projField) {
                if ($request->filled($projField)) {
                    $projectUpdates[$projField] = $request->input($projField);
                }
            }
            if (!empty($projectUpdates)) {
                CustomProject::where('id', $request->project_id)->update($projectUpdates);
            }

            $path = null;
            $fileSize = null;
            $originalName = $request->input('file_name');
            // Design-request message doubles as the dieline change note when no explicit comment is sent.
            $userComment = $request->input('comment')
                ?? $request->input('change_request_comment')
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        try {
            ...
            Log::info('Dieline status update triggered', ['id' => $id, 'data' => $request->all()]);

            $updateData = ['status' => $request->status];
            
            // Capture comment from either 'change_request_comment' or 'comment'
            $comment = $request->input('change_request_comment') ?? $request->input('comment');
            if ($comment) {
                $updateData['change_request_comment'] = $comment;
            }

            Log::debug('Dieline update data prepared', ['id' => $id, 'update_data' => $updateData]);
    ...
    /**
     * Request company to design a mockup (from mobile app)
     * Creates a pending record so CRM admin can see the request
     */
    public function requestCompanyMockup(Request $request, $dielineId)
    {
        try {
            $dieline = Dieline::findOrFail($dielineId);

            // Check if there's already a pending request
            $existing = Mockup::where('dieline_id', $dielineId)
                ->where('status', 'pending_company_design')
                ->first();

            if ($existing) {
                return response()->json([
                    'success' => true,
                    'message' => 'Company mockup already requested',
                    'data'    => ['id' => $existing->id, 'status' => 'pending_company_design'],
                ]);
            }

            $mockup = Mockup::create([
                'dieline_id'  => $dielineId,
                'file_name'   => 'Company Mockup Requested',
                'file_path'   => null,
                'file_size'   => null,
                'status'      => 'pending_company_design',
                'is_company'  => true,
            ...
            Log::info('Company mockup requested via app', [
                'dieline_id' => $dielineId,
                'mockup_id'  => $mockup->id,
            ]);
            ...
            return response()->json([
                'success' => true,
                'message' => 'Company mockup design requested successfully',
    ...
    public function updateMockupStatus(Request $request, $id)
    {
        ...
        Log::info('Mockup status update request data', [
            'mockup_id' => $id,
            'all_data' => $request->all(),
            'comment' => $request->input('comment'),
            'change_request_comment' => $request->input('change_request_comment')
        ]);
        ...
        try {
            ...
            Log::info('Mockup status update triggered', ['id' => $id, 'data' => $request->all()]);
            ...
            Log::debug('Mockup update data prepared', ['id' => $id, 'update_data' => $updateData]);
